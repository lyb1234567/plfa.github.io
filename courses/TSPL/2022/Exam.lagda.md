...
--begin
    data Tree : Set → Set where
        ...
        _branch_ : ∀ { A : Set } →  Tree A → Tree A → Tree A

    data AllT { A : Set} (P : A → Set) : Tree A → Set where
        
        leaf : ∀ { x : A } → P x → AllT P (leaf x) 
        _branch_ : ∀ { xt : Tree A} { yt : Tree A } → AllT P xt  → AllT P yt  → AllT P ( xt branch yt )
    ...
    data AnyT {A : Set} (P : A → Set) : Tree A → Set where
        ...
        left :  ∀ { xt : Tree A} { yt : Tree A }  → AnyT P xt → AnyT P ( xt branch yt )

        right :  ∀ { xt : Tree A} { yt : Tree A }  → AnyT P yt → AnyT P ( xt branch yt ) 
--end   
```
...
```agda
--begin
    AlltoAny : ∀ { A : Set }  {P : A → Set} (xt : Tree A) →  AllT (¬_ ∘ P ) xt →  ¬ (AnyT P xt)
    AlltoAny (leaf x) (leaf ¬Pxt ) = λ { (leaf lf) → ¬Pxt lf  }
    AlltoAny (xt branch yt ) (¬Allxt branch ¬Allyt) = λ { (left anyxt) → (AlltoAny xt ¬Allxt) anyxt  ; (right anyxt) → (AlltoAny yt ¬Allyt) anyxt   }
--end
```
...
```agda
  ...
  rename ρ (ok ⊢A)        =  ok (rename ρ ⊢A)
  rename ρ (letc ⊢A ⊢B)   =  μ (` Z)
  --end
```
...
```agda
  ...
  subst σ (ok ⊢A )       = ok (subst σ ⊢A)
  subst σ (letc ⊢A ⊢B )  = μ (` Z)
  --end
```
...
```agda
  ...
  data _—→_ : ∀ {Γ A} → (Γ ⊢ A) → (Γ ⊢ A) → Set where
    ...
    ζ-letc : ∀ {Γ A B} { M M' : Γ ⊢ Comp A} { N : Γ , A  ⊢ Comp B}
      → M —→ M'
      → (letc M N) —→ (letc M' N)

    β-error : ∀ {Γ A B } { t : Γ , A  ⊢ Comp B } { msg : String}
...
```agda
  ...
  progress (ok M)      with progress M 
  ...    | step M—→N                      = step (ζ-ok M—→N)
  ...    | done VM                        = done (V-ok VM)
  progress (letc ⊢A ⊢B) with progress ⊢A
  ...    | step M—→N                      = step (ζ-letc M—→N) 
  ...    | done (V-ok Vok)                = step (β-ok Vok)
...
```agda
  ...
  inherit Γ tt ⊤           = yes tt
  inherit Γ case⊤ L [tt⇒ M ] ⊤ with inherit Γ M ⊤
  ... | no ¬⊢N                =  no  (λ{ (case⊤ (⊢L) (⊢M)) → ¬⊢N ⊢M   } )
  ... | yes ⊢N with synthesize Γ L
  ... | no  ¬⊢M             =  no (λ{ (case⊤ (⊢L) (⊢M)) → ¬⊢M ⟨ ⊤ , ⊢L ⟩   })
  ... | yes ⟨ ⊤ , ⊢L ⟩             = yes (case⊤ ⊢L ⊢N) 
  ... | yes ⟨ `ℕ , ⊢L ⟩             = no ( λ{ (case⊤ (⊢L′) (⊢N)) → ℕ≢T (uniq-↑ ⊢L  ⊢L′ ) }  ) 
  ... | yes ⟨ _ ⇒ _ , ⊢L ⟩             = no ( λ{ (case⊤ (⊢L′) (⊢N)) → T≢⇒ (uniq-↑ ⊢L′ ⊢L ) }  )
  inherit Γ case⊤ L [tt⇒ M ] (A ⇒ B ) with inherit Γ M ( A ⇒ B )
  ... | no ¬⊢N                =  no  (λ{ (case⊤ (⊢L) (⊢M)) → ¬⊢N ⊢M   } )
  ... | yes ⊢N with synthesize Γ L
  ... | no  ¬⊢M             =  no (λ{ (case⊤ (⊢L) (⊢M)) → ¬⊢M ⟨ ⊤ , ⊢L ⟩   })
  ... | yes ⟨ ⊤ , ⊢L ⟩             = yes (case⊤ ⊢L ⊢N) 
  ... | yes ⟨ `ℕ , ⊢L ⟩             = no ( λ{ (case⊤ (⊢L′) (⊢N)) → ℕ≢T (uniq-↑ ⊢L  ⊢L′ ) }  ) 
  ... | yes ⟨ _ ⇒ _ , ⊢L ⟩             = no ( λ{ (case⊤ (⊢L′) (⊢N)) → T≢⇒ (uniq-↑ ⊢L′ ⊢L ) } )  
  inherit Γ case⊤ L [tt⇒ M ] ( `ℕ ) with inherit Γ M ( `ℕ )
  ... | no ¬⊢N                =  no  (λ{ (case⊤ (⊢L) (⊢M)) → ¬⊢N ⊢M   } )
  ... | yes ⊢N with synthesize Γ L
  ... | no  ¬⊢M             =  no (λ{ (case⊤ (⊢L) (⊢M)) → ¬⊢M ⟨ ⊤ , ⊢L ⟩   })
  ... | yes ⟨ ⊤ , ⊢L ⟩             = yes (case⊤ ⊢L ⊢N) 
  ...
  ... | yes ⟨ ⊤ ,  ⊢L ⟩       =   no   (λ{ (⊢case ⊢L′ _ _) →   ℕ≢T  (uniq-↑  ⊢L′ ⊢L  ) })
